...
plot_Rbiips_filtering_estimates <- function(df, pos = TRUE) {
...
# a: control input
# a[t-1] <- A * sin(two_pi*(t-1)/t_max)
p[t] ~ dnorm(p[t-1] + dt * w[t-1], prec_p)
w[t] ~ dnorm(w[t-1], prec_w)

y[t] ~ dnorm(w[t], prec_y)
}
}"

model_string <- model_string_control
...
plot_Rbiips_filtering_estimates(df, pos = TRUE)
